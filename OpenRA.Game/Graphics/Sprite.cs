...
#if !DEBG_ON
		public Sprite(Sheet sheet, Rectangle bounds, TextureChannel channel, Size size, BlendMode blendMode = BlendMode.Alpha)
		{
			Sheet = sheet;
			Bounds = bounds;
			Offset = float2.Zero;
			ZRamp = 0;
			Channel = channel;
			Size = new float3(size.Width, size.Height, 0);
			BlendMode = blendMode;
